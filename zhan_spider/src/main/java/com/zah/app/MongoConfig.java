...
public class MongoConfig {
    //@Value("${mongo.host}")
    @Value("${spring.data.mongodb.uri_gallery}")
    private String MONGO_URI_GALLERY;
    @Value("${spring.data.mongodb.uri_star}")
    private String MONGO_URI_STAR;
    ...
    public MongoDbFactory dbFactory1() throws UnknownHostException {
        return new SimpleMongoDbFactory(new MongoClientURI(MONGO_URI_GALLERY));
    }
...
*/
    ...
    @Bean
    public MongoDbFactory dbFactoryStar() throws UnknownHostException {
        return new SimpleMongoDbFactory(new MongoClientURI(MONGO_URI_STAR));
    }

    @Bean
    public MongoTemplate mongoTemplateStar() throws Exception {
        return new MongoTemplate(dbFactoryStar());
    }
}  